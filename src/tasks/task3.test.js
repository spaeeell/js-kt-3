...
test("выдается ошибка, если пользователь ввёл не число", () => {
    expect(() => renderLadder("abc")).toThrowError("Некорректные входные данные")
})

test("выдается ошибка, если высота отрицательная", () => {
    expect(() => renderLadder("-7")).toThrowError("Некорректные входные данные")
})

test("выдается ошибка, если высота не целая", () => {
    expect(() => renderLadder("7.12")).toThrowError("Некорректные входные данные")
})
